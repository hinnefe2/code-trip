...
**`producers/slack.py`** — Slack via the **claude.ai Slack MCP** (`https://mcp.slack.com/mcp`), invoked through the `claude` CLI by a small `ClaudeMCPClient` wrapper (`producers/claude_mcp.py`). The orchestrator never holds Slack credentials directly — auth piggy-backs on whatever the user already authorized in claude.ai, so there's no Slack app to install. Per poll tick (~30 s) it makes one tool call: `slack_search_public_and_private(query="<@{user_id}>", after=last_ts, sort="timestamp")`. That catches @-mentions in any channel (public, private, DM, group DM). Reply path uses `slack_send_message(channel_id, message, thread_ts)`. Token cost per call is ~$0.001 on Haiku with `--allowedTools` restricted to the one tool. **Limitation**: DMs that don't @-mention the user aren't currently surfaced — the MCP doesn't expose a clean "list-all-DMs-since-X" tool, so we lean on the mention search and accept the gap.
